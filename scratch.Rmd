...
create_feature_df <- function(dataset_name) {
  ...
  cgm <- load_file(str_c("~/a1c/data_gathering/data/", dataset_name, "/clean/cgm.csv"), CGM_HEADERS)
  
  a1c <- load_file(str_c("~/a1c/data_gathering/data/", dataset_name, "/clean/a1c.csv"), A1C_HEADERS)
  
  measurements <- load_file(str_c("~/a1c/data_gathering/data/", dataset_name, "/clean/measurements.csv"), NULL)
  
  combined <- combine_cgm_data_with_other_data(cgm, a1c, "a1c") %>% 
                combine_cgm_data_with_other_data(measurements, "measurements")
  
  filtered <- filter_all_cgm_by_a1c(combined)
...
Remove columns where more than 5% of numeric entries are NA?

```{r}
feature_df <- filter(feature_df, ! is.na(cgm_days) & cgm_days > 6)

feature_df <- mutate(feature_df, ea1c = 3.31 + mean_bg_full_day * 0.02392)
```

```{r}
feature_df <- feature_df %>% 
  select(-id, -cgm_days, -datafile, -study_start_date, -dataset, -cgm_name) %>% 
  na.omit()
```
