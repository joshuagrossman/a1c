...
load_and_filter <- function(data, write = FALSE) {
  ...
  # Don't need some columns. CGM name is usually the same for each
  # study, so study name captures that info. Daytime and nighttime
  # measurements are overly granular.
  feature_df <- select(data, -c("first_day_cgm", "last_day_cgm", 
                                "datafile", "cgm_name", 
                                "study_start_date", 
                                "years_since_diag", 
                                "metformin"),
  ...
  # Many a1c measurements appear to have less than a week of
  # associated cgm data. Studies desire at least a week of CGM data as
  # a lead-in or follow-up to an a1c measurement, typically.
  # table(data$cgm_days)
  ...
  # Arbitrarily choosing 5 days as minimum amount of CGM data required
  # allows us to capture additional ~1300 a1c measurements. Use 5.5%
  # as a minimum and 11.5% as a maximum to allow clean bucketing into
  # rounded a1cs.
  ...
  # In Protocol_H study, there are sometimes multiple HbA1c recordings
  # on the same day. This was likely to calibrate the a1c measurement.
  # Taking a random one from each day where there are multiple
  # recordings.
  feature_df <- feature_df %>% 
  ...
  # Some missing data in the CGM statistics. < 2%. Imputation from
  # other CGM columns seems reasonable. 20% of ages are missing.
  # Will use interaction w/ missing indicator. 
  ...
  feature_df <- feature_df %>% 
    mutate(race = if_else(race == "Black/African American", 
                          "Black", race),
  ...
  feature_df <- feature_df %>% 
    mutate(black = if_else(race == "Black", 1, 0),
           ...
           hispanic = if_else(ethnicity == "Hispanic or Latino", 
                              1, 0)) %>% 
    select(-race, -gender, -ethnicity)
  
  # # RT_CGM used datetimes instead of date, and supplied a weird
  # origin. # Don't need exact dates, just care about differences
  # between dates, so # using default origin since their origin cannot
  # be determined.
  feature_df <- feature_df %>%
