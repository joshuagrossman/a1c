# MSE226Part1Analysis.R
# Analyzes cgm, a1c, and demographic data for part 1.
...
#
# The code to create the best regression and classification model has
# been surrounded with 3 lines of hashes above and below. Hopefully
# easy to find with a quick scroll down!
#
######################################################################
...
# using simple median imputation. Missing ages were cast as mean(age)
# in `load_and_filter`, and will be accounted for later on using
# interaction term per Gelman/Hill chapter.
cleaned_df <- preProcess(cleaned_df, method = "medianImpute") %>%
  predict(cleaned_df) %>% 
  as.data.frame()

# Generate train and test data. Don't want the same patient to exist
# in the train and test data because multiple outcomes per patient.
training_ids <- 
  cleaned_df %>% 
  select(id) %>% 
  group_by(id) %>% 
...
# Baseline models for regression and classification. Current
# "gold-standard".
...
gmi_auc <- performance(prediction(gmi_unhealthy, 
                                  unhealthy_a1c), "auc")@y.values[[1]]
...
# SD of blood glucose and ethnicity look promising. Other covariates
# are almost too "specific", so will try to reduce RMSE to LASSO level
# by adding the most interpretable covariates as possible.
...
# LASSO performs just as well as LASSO^2 and RF. LM w/ race is pretty
# good though, and more "interpretable". LM w/ race and SD is also
# interpretable, and gets close to LASSO. Best model = MRS.
...
class_lasso2_fit <- train(unhealthy_a1c ~ .*.,
                    bind_cols(unhealthy_a1c = train_df$unhealthy_a1c, 
                              pre_processed_classification),
                    method = "glmnet",
                    family = "binomial",
                    tuneGrid = data.frame(alpha = 1,
                                          lambda = lambdas),
...
# Nothing is *that* much better than GMI in terms of accuracy. Again,
# though, a linear model with race/mean BG/sd BG works just as well as
# the more complicated models, so will call logistic MRS the best
# model.
...
train_df <- 
  mutate(train_df, 
         unhealthy_resid = 
           as.integer(unhealthy_a1c == predict(race_sd_glm_fit, 
                                               train_df)))
...
# Baseline models are GMI and simply predicting the same a1c as last
# time.
...
short_gmi_rmse <- 
  sqrt(mean((short_outcome_a1c - short_gmi)^2))
carryover_rmse <- 
  sqrt(mean((short_outcome_a1c - short_carryover_a1c)^2))
...
# Best model augmented with the previous a1c value and the residual
# from previous prediction. (MRS++)
...
# Including the previous a1c substantially improves the RMSE. Adding
# the residual doesn't do much beyond that.
...
short_train_df %>% 
  mutate(gmi_err = a1c_value - 
           (3.31 + 0.02392 * mean_bg_full_day)) %>% 
  group_by(dataset) %>% 
  summarize(n = n(),
...
cross_fit_control <- trainControl(method = "repeatedcv",
                                  index = groupKFold(train_df$dataset, 
                                                     5),
                                  verboseIter = FALSE,
                                  allowParallel = TRUE,
                                  classProbs = TRUE,
...
pred <- predict(roc_fit, newdata = roc_validation_df, 
                type = "response")
...
#
# # Baseline models are GMI and using the same classification as the
# last a1c.
# short_gmi_unhealthy <- factor(as.integer(short_gmi < 7.5))
# short_carryover_unhealthy <- 
#   factor(as.integer(short_carryover_a1c < 7.5))
# short_unhealthy_a1c <- short_train_df$unhealthy_a1c

# 
# # short_gmi_acc <- mean(short_unhealthy_a1c == short_gmi_unhealthy)
# carryover_acc <- 
#   mean(short_unhealthy_a1c == short_carryover_unhealthy)
# # 
# # Best model from before.
# class_former_fit <-
  # train(unhealthy_a1c ~ 1 + mean_bg_full_day + black + 
  #         sd_bg_full_day,
  #       short_train_df,
  #       method = "glm",
  #       family = binomial(),
...
#   train(unhealthy_a1c ~ 1 + last_unhealthy_a1c + mean_bg_full_day + 
#           black + sd_bg_full_day + last_unhealthy_resid,
#         short_train_df,
#         method = "glm",
#         family = binomial(),
#         trControl = short_fit_control)
...
# Interesting, GMI is no worse than the best model that does not
# include the # prior a1c.
...
#              resid_last_a1c_former_cv_acc = 
#                resid_last_a1c_class_former_fit),
#         extract_cv_accuracy))


# fold_lengths <- map_int(mrs_class_cross_fit$control$indexOut, 
#                         length)
# 
...
#   inner_join(tibble(`MRS_ACC` = cv_acc, 
# n = fold_lengths), by = "n") %>% 
