...
# source("feature_extraction/cgm_feature_extraction.R")
# source("feature_extraction/merging_cgm_data.R")
# 
# options(digits = 3,
#         scipen = 999)
```
...
#   
#   combined <- 
#     combine_cgm_data_with_other_data(cgm, a1c, 
#                                      other_data_name = "a1c") %>% 
...
```{r}
# use this line to regenerate feature_df
...
#   
#   # In protocol H, there are multiple HbA1c recordings on the same day
#   # This was likely to calibrate. Taking the "first" one from each day.
...
```{r}
# print(paste("Number of patients:", length(unique(feature_df$id))))
# print(paste("Number of HbA1c records:", length(feature_df$id)))
# 
# mean_sd_min_max <- function(name) {
#   v <- pull(feature_df, name)
#   print(paste("Mean/SD/Min/Max ", name, na_mean(v), na_sd(v), min(v, na.rm = T), max(v, na.rm = T)))
# }
# mean_sd_min_max("cgm_days")
# mean_sd_min_max("a1c_count")
# mean_sd_min_max("a1c_value")
# mean_sd_min_max("age")
# 
# patients <- feature_df %>% 
#   group_by(id) %>% 
#   slice(1)
# 
# table(patients$race) / nrow(patients)
# table(patients$ethnicity) / nrow(patients)
# table(patients$gender) / nrow(patients)
```


```{r}
# feature_df <- feature_df %>% mutate(#bmi = weight / (height / 100)^2,
#                        
#                        
...
```{r}
# # using for gmi later on
# mean_bg_full_day <- feature_df$mean_bg_full_day
# 
# feature_df <- feature_df %>% 
#   select(-first_day_cgm, 
#          -last_day_cgm, 
#          
#          # very few patients are hispanic, so removing bc near zero variance.
#          -ethnicity,
# 
#          -datafile, 
#          -study_start_date, 
...
```{r}
# write_csv(feature_df, "cleaned_features.csv")
```

```{r}
dummied <- 
  dummyVars(a1c_value ~ . - id - a1c_date, feature_df) %>% 
  predict(feature_df)
```
...
#   right_join(unique_patients, by = "id") %>% 
#   pull("index")
# 
...
lm_race_fit <- lm(outcome_a1c ~ . , lm_df[training, ])

lm_race_pred <- predict(lm_race_fit, lm_df)
```
...
(lm_race_results <- postResample(lm_race_pred[-training], test_outcome))
```
...
```{r}
# DEFINE CV FOLDS EXPLICITLY -- DONE used groupKFold

fit_control <- trainControl(method = "repeatedcv",
                            index = groupKFold(feature_df[training,]$id, 5),
                            verboseIter = FALSE,
                            allowParallel = TRUE)
```
...
```{r}
xgb_pred <- predict(xgb_fit, test_df)
(xgb_results <- postResample(xgb_pred, test_outcome))
```
...
```{r}
xgb_pred_2 <- predict(xgb_fit_2, test_df)
(xgb_results_2 <- postResample(xgb_pred_2, test_outcome))
```
...
prediction_df <- 
  ...
  add_predictions(lasso_fit, var = "lasso_pred_no_race") 
  #add_predictions(lasso_fit, var = "lasso2_pred") %>% 
  #add_predictions(rf_fit, var = "rf_pred") %>% 
  #add_predictions(xgb_fit, var = "xgb_pred")
...
prediction_df %>% 
  ...
  geom_point(aes(x = lasso_pred, y = outcome_a1c), color = "blue", alpha = 0.5) +
  theme_light() +
  xlab("Predicted HbA1c") +
  ylab("Actual HbA1c") +
  ggtitle("Red = GMI, Blue = LASSO") +
  geom_abline() +
  ggsave("together.png")
```
